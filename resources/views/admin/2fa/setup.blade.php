...
<div class="max-w-2xl mx-auto">
    ...
    <div class="grid gap-6">
            ...
            <div class="flex flex-col items-center gap-4">
                ...
                <div class="p-4 bg-white rounded-xl">
                    <div class="w-48 h-48 bg-slate-200 flex items-center justify-center text-slate-400 text-xs text-center p-4">
                        QR Code akan ditampilkan di sini.<br>
                        URL: {{ $qrCodeUrl }}
                    </div>
                </div>
                
                <div class="text-center">
                    <p class="text-xs text-slate-500 mb-1">Atau masukkan secret key secara manual:</p>
                    <code class="px-3 py-1 bg-slate-800 rounded text-sm font-mono">{{ substr(Auth::guard('admin')->user()->two_factor_secret, 0, 16) }}...</code>
                </div>
            </div>
        </div>
        ...
        <div class="bg-surface-dark rounded-2xl border border-slate-800 p-6">
            ...
            <button onclick="copyRecoveryCodes()" class="w-full py-2 border border-slate-700 hover:bg-slate-800 rounded-lg text-sm transition flex items-center justify-center gap-2">
            ...
            <form method="POST" action="{{ route('admin.2fa.enable') }}">
                ...
                <div class="flex gap-2 mb-4">
                    <input type="text" name="code" maxlength="6"
                           class="flex-1 px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-center text-2xl font-bold tracking-widest focus:border-primary focus:outline-none"
                           placeholder="000000"
                           inputmode="numeric"
                           pattern="[0-9]*"
                           required>
                ...
                <div class="flex gap-3">
                    <a href="{{ route('admin.settings.index') }}" class="flex-1 py-3 border border-slate-700 hover:bg-slate-800 rounded-xl font-medium text-center transition">
                        Batal
                    </a>
