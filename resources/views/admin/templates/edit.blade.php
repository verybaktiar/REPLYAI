...
@section('page_title', 'Edit Message Template')
...
<div class="max-w-4xl mx-auto">
    ...
    <form action="{{ route('admin.templates.update', $template) }}" method="POST" class="space-y-6" x-data="templateEditor()">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 bg-surface-dark rounded-2xl border border-slate-800 p-6">
                ...
                <textarea name="content" x-ref="content" x-model="content" @input="updatePreview()" required
                          ...
                          placeholder="Enter your message template here...">{{ old('content', $template->content) }}</textarea>
                @error('content')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
                ...
                <div class="mt-4">
                    ...
                    <input type="text" name="variables" value="{{ old('variables', implode(', ', $template->variables)) }}"
        ...
        <div class="bg-surface-dark rounded-2xl border border-slate-800 p-6">
            ...
            <label class="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $template->is_active) ? 'checked' : '' }} 
                       class="w-5 h-5 rounded border-slate-600 text-primary focus:ring-primary bg-surface-light">
        ...
        <div class="flex items-center justify-between">
            <form action="{{ route('admin.templates.destroy', $template) }}" method="POST" 
                ...
                @csrf
                @method('DELETE')
                <button type="submit" class="px-6 py-3 bg-red-600 hover:bg-red-700 rounded-xl font-semibold transition flex items-center gap-2">
...
function templateEditor() {
    return {
        content: '{{ addslashes(old('content', $template->content)) }}',
        preview: '',
        detectedVariables: [],
