        ...
        <header class="h-16 flex items-center justify-between px-6 lg:px-8 border-b border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-background-dark/50 backdrop-blur-sm sticky top-0 z-20">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400">Buat sequence pesan otomatis baru</p>
                </div>
            </div>
        </header>
        ...
        <div class="flex-1 overflow-y-auto p-6 lg:p-8 scroll-smooth">
            <div class="max-w-3xl mx-auto">
                <form action="{{ route('sequences.store') }}" method="POST" id="sequence-form">
                    ...
                    <!-- Basic Info Card -->
                    <div class="bg-white dark:bg-surface-dark rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm mb-6">
                        ...
                        <div class="p-6 space-y-5">
                            ...
                            <div>
                                ...
                                <textarea name="description" rows="2"
                                          class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-3 dark:text-white text-slate-900 placeholder-slate-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-primary/50 focus:border-primary/50"
                                          placeholder="Deskripsi singkat tentang sequence ini...">{{ old('description') }}</textarea>
                            </div>
                            ...
                            <div id="trigger-value-container" class="hidden">
                                <label class="block text-sm font-medium dark:text-white text-slate-900 mb-2" id="trigger-value-label">Keyword</label>
                                <input type="text" name="trigger_value" value="{{ old('trigger_value') }}"
                                       class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-3 dark:text-white text-slate-900 placeholder-slate-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-primary/50 focus:border-primary/50"
                            ...
                            <div class="flex items-center gap-3">
                                <input type="checkbox" name="is_active" value="1" checked
                                       class="rounded bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-primary focus:ring-primary">
                                <label class="text-sm dark:text-white text-slate-900">Aktifkan sequence ini segera</label>
                            </div>
                        </div>
                    </div>
...
<template id="step-template">
    <div class="step-item bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700/50 p-4" data-step-index="">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <select name="steps[INDEX][delay_type]" 
                                class="w-full bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:text-white text-slate-900 focus:ring-2 focus:ring-primary/50 focus:border-primary/50">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        addStep();
        // Trigger change to show/hide trigger value
